										...
										_data.delete('checks', checkId, function(err){
											...
											if(checksDeleted == checksToDelete){
												if(!deletionErrors){
													...
													callback(500, {'Error' : 'Errors encountered when attempting to delete the user\'s checks'});
												}
											}
										});
									});
		...
		_data.read('tokens', token, function(err, tokenData){
			if(!err && tokenData){
				...
				_data.read('users', userPhone, function(err, userData){
					if(!err && userData){
						let userChecks = typeof(userData.checks) == 'object' && userData.checks instanceof Array ? userData.checks : [];
						// Verify number of maxChecks of user
                ...
                'timeoutSeconds' : timeoutSeconds
							...
							_data.create('checks', checkId, checkObject, function(err){
								if(!err){
									...
									callback(500, {'Error' : 'Culd not create the new check'});
								}
							});
						} else {
							callback(400, {'Error' : 'The user has the max amount of checks('+config.maxChecks+')'});
						}
					} else {
						callback(403);
					}
...
handlers._checks.put = function(data, callback){
	...
	if(id){
		// Check at least one optional data has been sent
		if(protocol || url || method || successCodes || timeoutSeconds){
			// Lookup the check
			_data.read('checks', id, function(err, checkData){
				if(!err && checkData){
								...
								_data.read('users', checkData.userPhone, function(err, userData){
									if(!err && userData){
										...
										if(checkPosition > -1){
											...
											callback(500, {'Error' : 'Could not find the check on user object, so could not remove it'});
										}
									} else {
										callback(500, {'Error' : 'Could not find the user who created the check, so could not remove check from user list'});
									}
								});
							} else {
